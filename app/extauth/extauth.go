...
// New creates an Instance configured from cfg.
// The Instance's timeout, heartbeat, and ttl are converted from seconds to time.Duration,
// and the disconnect flag is applied from cfg. New always returns a non-nil *Instance and a nil error.
...
// init registers the extauth Config type with the common configuration registry and panics if registration fails.
func init() {
